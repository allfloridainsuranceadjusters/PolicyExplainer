...
async function callClaudeJson({ system, user, model = "claude-sonnet-4-20250514", temperature = 0.2 }) {
    ...
    try {
        ...
        if (jsonMatch) {
            try { parsed = JSON.parse(jsonMatch[1].trim()); } catch (e2) { throw new Error(`Claude JSON parse failed. First 300 chars: ${content.slice(0, 300)}`); }
        } else {
            // Try to find JSON object in the response
            const braceMatch = content.match(/\{[\s\S]*\}/);
            if (braceMatch) {
                try { parsed = JSON.parse(braceMatch[0]); } catch (e3) { throw new Error(`Claude JSON parse failed. First 300 chars: ${content.slice(0, 300)}`); }
            } else {
